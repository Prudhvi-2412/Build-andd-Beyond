...
const { Worker, ArchitectHiring, DesignRequest, Transaction, ConstructionProjectSchema, Company } = require('../models');
const { invalidateCacheByPrefix } = require('../utils/redisCache');

const COMPANY_PLATFORM_FEE_PERCENT = parseFloat(process.env.COMPANY_PLATFORM_FEE_PERCENT || process.env.PLATFORM_FEE_PERCENT) || 5;
const COMPANY_INITIAL_RELEASE_PERCENT = 75;
const COMPANY_HOLD_PERCENT = 25;
const WORKER_PLATFORM_FEE_PERCENT = 5;
const RAZORPAY_MAX_ORDER_AMOUNT_INR = Number(process.env.RAZORPAY_MAX_ORDER_AMOUNT_INR || 500000);
...
const appendCustomerMilestoneNotification = (project, milestonePercentage, message, now = new Date()) => {
  const milestone = (project.milestones || []).find(
    (entry) => Number(entry.percentage) === Number(milestonePercentage) && entry.isCheckpoint
  );
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        nextPaymentDue: 'When Milestone 1 (25%) is approved, held amount is released. Platform fee must be collected before the next milestone payment.'
      }
    });
...
const collectMilestonePayment = async (req, res) => {
  try {
    ...
    if (milestoneIndex > 0) {
      const previousMilestone = project.paymentDetails.milestonePayments[milestoneIndex - 1];
...
const createWorkerPaymentOrder = async (req, res) => {
  try {
    const { projectId, projectType, paymentType, milestonePercentage } = req.body;

    if (!projectId || !projectType || !paymentType) {
      return res.status(400).json({ success: false, message: 'projectId, projectType and paymentType are required' });
    }
    ...
    if (!['deposit', 'milestone'].includes(paymentType)) {
      return res.status(400).json({ success: false, message: 'paymentType must be deposit or milestone' });
    }

    const Project = projectType === 'architect' ? ArchitectHiring : DesignRequest;
    const project = await Project.findById(projectId);
    if (!project) {
    ...
    if (!project.paymentDetails || project.paymentDetails.escrowStatus === 'not_initiated') {
      return res.status(400).json({ success: false, message: 'Escrow not initialized for this project' });
    }

    const targetPercentage = paymentType === 'deposit' ? 25 : Number(milestonePercentage);
    if (!MILESTONE_PERCENTAGES.includes(targetPercentage)) {
      return res.status(400).json({ success: false, message: 'Invalid milestone percentage. Must be 25, 50, 75, or 100' });
    }
    ...
    if (!milestonePayment) {
      return res.status(404).json({ success: false, message: 'Milestone payment not found' });
    }

    if (milestonePayment.paymentCollected) {
      return res.status(400).json({ success: false, message: `Payment for ${targetPercentage}% milestone has already been collected` });
    }
    ...
    const amount = Math.round(Number(milestonePayment.amount || 0));
    if (amount <= 0) {
      return res.status(400).json({ success: false, message: 'Invalid milestone amount configured' });
    }
    ...
    return res.status(500).json({ success: false, message: 'Failed to create worker payment order', error: error.message });
  }
};
...
const verifyWorkerPayment = async (req, res) => {
  try {
    ...
    if (!projectId || !projectType || !paymentType || !razorpay_order_id || !razorpay_payment_id || !razorpay_signature) {
      return res.status(400).json({
        success: false,
        message: 'projectId, projectType, paymentType and Razorpay verification fields are required',
      });
    }
    ...
    return res.status(500).json({ success: false, message: 'Failed to verify worker payment', error: error.message });
  }
};
...
const createCompanyPaymentOrder = async (req, res) => {
  try {
    ...
    if (!projectId || !milestonePercentage) {
      return res.status(400).json({ success: false, message: 'projectId and milestonePercentage are required' });
    }
    if (![25, 50, 75, 100].includes(targetMilestone)) {
      return res.status(400).json({ success: false, message: 'milestonePercentage must be 25, 50, 75, or 100' });
    }

    const project = await ConstructionProjectSchema.findById(projectId);
    if (!project) return res.status(404).json({ success: false, message: 'Project not found' });
    if (project.status !== 'accepted') {
      return res.status(400).json({ success: false, message: 'Payments can only be made on accepted projects' });
    }

    const resolvedPhase = resolveCompanyPhase(project, milestonePercentage);
    if (!resolvedPhase) {
      return res.status(400).json({ success: false, message: 'Phase configuration not found for this milestone' });
    }
    ...
    if (amount <= 0) {
      return res.status(400).json({ success: false, message: 'Phase amount is not configured' });
    }
    ...
    if (previousMilestone && !existingPayout) {
      ...
      if (!previousPayout || previousPayout.status !== 'released' || previousPayout.platformFeeStatus !== 'collected') {
        return res.status(400).json({
          success: false,
    ...
    const immediateReleaseAmount = roundCurrency((amount * COMPANY_INITIAL_RELEASE_PERCENT) / 100);
    const completionAmount = roundCurrency(amount - immediateReleaseAmount);
    const platformFee = roundCurrency((amount * COMPANY_PLATFORM_FEE_PERCENT) / 100);
    ...
    if (!existingPayout) {
      ...
      amountToCharge = Math.min(stageRemainingAmount, RAZORPAY_MAX_ORDER_AMOUNT_INR);
    } else if (existingPayout.status === 'pending') {
      const remainingUpfrontAmount = roundCurrency(
        Number(existingPayout.immediateReleaseAmount || 0) - Number(existingPayout.customerPaidAmount || 0)
      );
      ...
      const milestone = (project.milestones || []).find(
        (entry) => Number(entry.percentage) === targetMilestone && entry.isCheckpoint
      );
      ...
      const remainingAmount = roundCurrency(Number(existingPayout.amount || 0) - Number(existingPayout.customerPaidAmount || 0));
      if (remainingAmount <= CURRENCY_EPSILON) {
        return res.status(400).json({ success: false, message: `Remaining payment for ${targetMilestone}% phase is already completed` });
      }
    ...
    if (amountToCharge <= CURRENCY_EPSILON) {
      return res.status(400).json({ success: false, message: 'Computed payable amount is invalid for this phase' });
    }
    ...
    try {
      ...
      const gatewayCode = gatewayError?.error?.code || gatewayError?.code || null;
    ...
    res.json({
      ...
      data: {
        ...
        remainingAfterThisOrder: roundCurrency(stageRemainingAmount - amountToCharge),
        installmentCountForCurrentStage: Math.max(1, Math.ceil(stageRemainingAmount / RAZORPAY_MAX_ORDER_AMOUNT_INR)),
        paymentStage,
...
const verifyCompanyPayment = async (req, res) => {
  try {
    ...
    if (!isTestBypass && (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature)) {
      return res.status(400).json({ success: false, message: 'projectId, razorpay_order_id, razorpay_payment_id and razorpay_signature are required' });
    }

    if (isTestBypass && process.env.NODE_ENV === 'production') {
      return res.status(403).json({ success: false, message: 'Test payment bypass is disabled in production' });
    }
    ...
    const payout = isTestBypass
      ? project.paymentDetails.payouts.find(
          (p) => Number(p.milestonePercentage) === requestedMilestone && ['pending', 'held'].includes(p.status)
        )
      : project.paymentDetails.payouts.find(
          (p) => p.razorpayOrderId === razorpay_order_id && ['pending', 'held'].includes(p.status)
        );
    if (!payout) {
      return res.status(404).json({ success: false, message: 'Matching payout order not found' });
    }

    const effectiveMilestone = Number(milestonePercentage || payout.milestonePercentage);
    const now = new Date();
    const milestone = (project.milestones || []).find(
    ...
    if (isTestBypass) {
      ...
      try {
        ...
        paidChunkAmount = roundCurrency(Number(orderDetails?.amount || 0) / 100);
      } catch (orderFetchError) {
        console.warn('Unable to fetch Razorpay order during verification, using computed fallback amount.', {
    ...
    if (paidChunkAmount <= CURRENCY_EPSILON) {
      const fallbackRemaining = roundCurrency(Number(payout.amount || 0) - Number(payout.customerPaidAmount || 0));
      paidChunkAmount = fallbackRemaining;
    }

    if (paidChunkAmount <= CURRENCY_EPSILON) {
      return res.status(400).json({ success: false, message: 'Unable to determine paid amount for this payment' });
    }

    const totalPhaseAmount = roundCurrency(Number(payout.amount || 0));
    const upfrontTarget = roundCurrency(
      Number(payout.immediateReleaseAmount || (totalPhaseAmount * COMPANY_INITIAL_RELEASE_PERCENT) / 100)
    );
    ...
    const transactionPaymentId = razorpay_payment_id || `test_payment_${Date.now()}`;
    ...
    if (payout.status === 'pending') {
      if (upfrontTarget <= CURRENCY_EPSILON) {
        return res.status(400).json({ success: false, message: 'Invalid initial amount configured for this phase' });
      }

      const remainingUpfront = roundCurrency(upfrontTarget - Number(payout.customerPaidAmount || 0));
      const appliedUpfrontAmount = roundCurrency(Math.min(remainingUpfront, paidChunkAmount));

      if (appliedUpfrontAmount <= CURRENCY_EPSILON) {
        return res.status(400).json({ success: false, message: 'Initial payment is already complete for this phase' });
      }

      payout.razorpayPaymentId = razorpay_payment_id;
      payout.customerPaidAt = now;
      payout.initialReleaseDate = now;
      payout.customerPaidAmount = roundCurrency(Number(payout.customerPaidAmount || 0) + appliedUpfrontAmount);
      payout.platformFeeStatus = 'not_due';

      const remainingAfterUpfront = roundCurrency(upfrontTarget - Number(payout.customerPaidAmount || 0));
      payout.status = remainingAfterUpfront <= CURRENCY_EPSILON ? 'held' : 'pending';

      releasedAmount = appliedUpfrontAmount;
      project.paymentDetails.amountPaidToCompany =
        roundCurrency(Number(project.paymentDetails.amountPaidToCompany || 0) + releasedAmount);
      if (milestone && milestone.payments) {
        const paidUpfrontSoFar = roundCurrency(Math.min(Number(payout.customerPaidAmount || 0), upfrontTarget));
        milestone.payments.upfront.amount = paidUpfrontSoFar;
        milestone.payments.upfront.status = paidUpfrontSoFar >= upfrontTarget ? 'released' : 'pending';
      ...
      const remainingCompletionAmount = roundCurrency(Number(payout.amount || 0) - Number(payout.customerPaidAmount || 0));
      if (remainingCompletionAmount <= CURRENCY_EPSILON) {
        return res.status(400).json({ success: false, message: 'Remaining amount is already paid for this phase' });
      }

      const appliedCompletionAmount = roundCurrency(Math.min(remainingCompletionAmount, paidChunkAmount));
      if (appliedCompletionAmount <= CURRENCY_EPSILON) {
        return res.status(400).json({ success: false, message: 'Unable to apply installment for completion payment' });
      }

      payout.razorpayPaymentId = razorpay_payment_id;
      payout.customerPaidAt = payout.customerPaidAt || now;
      payout.customerPaidAmount = roundCurrency(Number(payout.customerPaidAmount || 0) + appliedCompletionAmount);

      const remainingAfterCompletion = roundCurrency(Number(payout.amount || 0) - Number(payout.customerPaidAmount || 0));
      payout.status = remainingAfterCompletion <= CURRENCY_EPSILON ? 'released' : 'held';
      ...
      if (milestone && milestone.payments) {
        ...
        milestone.payments.completion.status = paidCompletionSoFar >= completionTarget ? 'released' : 'pending';
    ...
    const allReleased = project.paymentDetails.payouts.every((p) => p.status === 'released');
    const allFeesCollected = project.paymentDetails.payouts.every(
      (p) => ['collected', 'not_due'].includes(p.platformFeeStatus || 'not_due')
    );
    project.paymentDetails.paymentStatus = allReleased && allFeesCollected ? 'completed' : 'partially_paid';
    ...
    res.json({
      ...
      data: {
        ...
        remainingAmount: roundCurrency(Number(payout.amount || 0) - Number(payout.customerPaidAmount || 0)),
...
const releaseCompanyMilestonePayment = async (req, res) => {
  try {
    ...
    if (!payout) {
      return res.status(404).json({ success: false, message: 'No payment record found for this milestone.' });
    }
    ...
    const remainingAmount = roundCurrency(Number(payout.amount || 0) - Number(payout.customerPaidAmount || 0));
    ...
    const milestone = project.milestones.find(m => m.percentage === milestonePercentage && m.isCheckpoint);
    if (milestone && milestone.payments) {
    ...
    project.paymentDetails.amountPaidToCompany = (project.paymentDetails.amountPaidToCompany || 0) + holdAmount;
    ...
    if (allReleased && allFeesCollected) project.paymentDetails.paymentStatus = 'completed';
    ...
    res.status(500).json({ success: false, message: 'Failed to release payout', error: error.message });
  }
};
...
const getPendingCompanyPlatformFees = async (req, res) => {
  try {
    ...
    projects.forEach((project) => {
      (project.paymentDetails?.payouts || []).forEach((payout) => {
        ...
        items.push({
          ...
          paymentStatus: project.paymentDetails?.paymentStatus || 'partially_paid',
        });
      });
    });
    ...
    res.status(500).json({ success: false, message: 'Failed to fetch platform fee queue', error: error.message });
  }
};
...
const collectCompanyPlatformFee = async (req, res) => {
  try {
    ...
    if (!payout || payout.status !== 'released' || payout.platformFeeStatus !== 'pending') {
      return res.status(400).json({ success: false, message: 'No pending platform fee found for this phase' });
    }
    ...
    res.status(500).json({ success: false, message: 'Failed to collect platform fee', error: error.message });
  }
};
...
const createCompanyPlatformFeeOrder = async (req, res) => {
  try {
    ...
    if (!requesterId || String(project.companyId) !== requesterId) {
      return res.status(403).json({ success: false, message: 'Not authorized to pay platform fee for this project' });
    }
    ...
    if (!payout || payout.status !== 'released') {
      return res.status(400).json({ success: false, message: 'Platform fee is available only after phase payout release' });
    }
    ...
    if (payout.platformFeeStatus === 'collected') {
      return res.status(400).json({ success: false, message: 'Platform fee is already paid for this phase' });
    }

    const platformFeeAmount = roundCurrency(Number(payout.platformFee || 0));
    if (platformFeeAmount <= 0) {
      return res.status(400).json({ success: false, message: 'Platform fee amount is invalid for this phase' });
    }
    ...
    res.status(500).json({ success: false, message: 'Failed to create platform fee order', error: error.message });
  }
};
...
const verifyCompanyPlatformFeePayment = async (req, res) => {
  try {
    ...
    if (!isTestBypass && (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature)) {
      return res.status(400).json({
        success: false,
        message: 'projectId, milestonePercentage, razorpay_order_id, razorpay_payment_id and razorpay_signature are required',
      });
    }
    ...
    const requesterId = req.user?.user_id ? String(req.user.user_id) : null;
    if (!requesterId || String(project.companyId) !== requesterId) {
      return res.status(403).json({ success: false, message: 'Not authorized to verify platform fee for this project' });
    }

    ensureCompanyPaymentDetails(project);

    const payout = findCompanyPayout(project, milestonePercentage);
    if (payout && payout.status === 'released' && Number(payout.platformFee || 0) > 0 && payout.platformFeeStatus === 'not_due') {
      payout.platformFeeStatus = 'pending';
    }
    ...
    if (!isTestBypass && payout.platformFeeRazorpayOrderId && payout.platformFeeRazorpayOrderId !== razorpay_order_id) {
      return res.status(400).json({ success: false, message: 'Order mismatch for this platform fee payment' });
    }

    const transactionOrderId = razorpay_order_id || `test_pf_order_${Date.now()}`;
    const transactionPaymentId = razorpay_payment_id || `test_pf_payment_${Date.now()}`;
    ...
    res.status(500).json({ success: false, message: 'Failed to verify platform fee payment', error: error.message });
  }
};
...
const getCompanyProjectPaymentSummary = async (req, res) => {
  try {
    ...
    const project = await ConstructionProjectSchema.findById(projectId)
      .select('projectName proposal paymentDetails milestones customerId companyId status');
    if (!project) return res.status(404).json({ success: false, message: 'Project not found' });

    const transactions = await Transaction.find({ projectId, projectType: 'construction' })
      .sort({ createdAt: -1 });
    ...
    const totalReleased = project.paymentDetails.payouts
      ...
      .reduce((s, p) => s + Number((p.immediateReleaseAmount || 0) + (p.heldAmount || 0)), 0);
    ...
    res.json({
      ...
      data: {
        projectName: project.projectName,
        status: project.status,
        totalProjectAmount: project.paymentDetails.totalAmount || project.proposal?.price || 0,
        platformFeePercent: COMPANY_PLATFORM_FEE_PERCENT,
